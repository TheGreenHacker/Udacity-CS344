  ...
  if (idx_2D.x < numCols && idx_2D.y < numRows) {
      ...
      for (int filter_r = -filterWidth/2; filter_r <= filterWidth/2; filter_r++) {
        for (int filter_c = -filterWidth/2; filter_c <= filterWidth/2; filter_c++) {
          ...
          int image_r = min(max(idx_2D.y + filter_r, 0), numRows - 1);
          int image_c = min(max(idx_2D.x + filter_c, 0), numCols - 1);
      ...
      outputChannel[idx_2D.y * numCols + idx_2D.x] = result;
  }
}
...
void cleanup() {
  ...
  checkCudaErrors(cudaFree(d_filter));  // allocated filter
}
